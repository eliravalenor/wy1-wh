class APIService {
    ...
    normalizeEndpoint(apiUrl) {
        // 如果只是填到了 /v1，则自动补到 /chat/completions
    ...
    async callOpenAIAPI(apiUrl, apiKey, model, messages, options = {}, timeout = 60000) {
        ...
        for (let i = 0; i < this.maxRetries; i++) {
            try {
                ...
                const response = await fetch(this.normalizeEndpoint(apiUrl), {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    try {
                        ...
                        throw new Error(`请求失败: ${response.status} - ${errorBody.error}`);
                    } catch (parseError) {
    ...
    async testConnection(apiUrl, apiKey) {
        ...
        if (!response.ok) {
            // 502/503/504 重试（外层已有重试循环，此处抛错让外层处理）
